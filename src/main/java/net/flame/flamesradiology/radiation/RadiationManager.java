...
public class RadiationManager {
    ...
    public static double calculateRadiationAt(Level level, BlockPos position) {
        ...
        for (RadiationSource source : sources) {
            double sourceRadiation = source.getRadiationAt(position);
            // Apply lead shielding
    ...
    public static double getTotalRadiation(Level level, Player player) {
        ...
        List<RadiationSource> sources = findRadiationSources(level, playerPos, MAX_SCAN_RADIUS);
        double environmentalRadiation = 0.0;
        
        for (RadiationSource source : sources) {
            double sourceRadiation = source.getRadiationAt(playerPos);
            // Apply lead shielding for each source
            double shieldedRadiation = LeadShielding.calculateShieldedRadiation(
                level, source.getPosition(), playerVec, sourceRadiation);
            environmentalRadiation += shieldedRadiation;
        }
